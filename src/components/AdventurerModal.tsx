    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-3xl bg-[#1c1917] border border-yellow-900/50 shadow-2xl overflow-hidden rounded-md font-adventurer"
        >
          ...
          <div className="flex flex-col md:flex-row h-full max-h-[85vh]">
            {/* Left: Artifact Visual with Lens Effect */}
            <div className="w-full md:w-1/2 relative h-56 md:h-auto border-b md:border-b-0 md:border-r border-yellow-900/20 overflow-hidden">
              <Image 
                src={item.image || item.backgroundImage || '/images/placeholder.jpg'} 
                alt={item.title} 
                fill 
                className={`object-cover transition-all duration-1000 ${loadingDiscovery ? 'blur-md grayscale scale-110' : 'blur-0 grayscale-0 scale-100'}`}
              />
            ...
            <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col overflow-y-auto no-scrollbar bg-[#1c1917]">
              {loadingDiscovery ? (
                ...
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                  ...
                  <h2 className="text-3xl font-black text-stone-200 mb-4 uppercase tracking-tighter italic">
                    {item.title}
                  </h2>

                  <div className="flex gap-4 mb-8">
                    <div className="flex items-center gap-2 text-[10px] text-stone-500 uppercase tracking-widest border border-stone-800 px-2 py-1">
                  ...
                  <p className="text-stone-400 leading-relaxed italic text-sm mb-8 border-l border-yellow-900/50 pl-4 py-1">
                    "{item.description}"
                  </p>
                    ...
                    <div className="mt-auto space-y-3">
                        {item.status === 'in-progress' ? (
                            ...
                            <a 
                                ...
                                className="w-full py-4 flex items-center justify-center gap-3 bg-yellow-800 text-stone-100 font-black uppercase text-[10px] tracking-[0.2em] hover:bg-yellow-700 transition-all shadow-xl shadow-black/50"
                            >
                            ...
                            <a 
                                ...
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="w-full py-3 flex items-center justify-center gap-3 border border-yellow-900/40 text-yellow-700 font-bold uppercase text-[9px] tracking-[0.1em] hover:bg-yellow-900/10 transition-all"
                            >
