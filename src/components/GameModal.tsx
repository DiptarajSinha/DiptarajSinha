    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className={`relative w-full max-w-lg lg:max-w-4xl overflow-hidden flex flex-col lg:flex-row min-h-[500px] ${
            isStalker 
        ...
        >
          {/* Left side: Image (Only for Adventurer or if image exists) */}
          {game.image && (
            <div className={`w-full lg:w-1/2 relative bg-black flex items-center justify-center p-4 min-h-[250px] lg:min-h-0 ${isStalker ? 'border-b lg:border-b-0 lg:border-r border-red-900/50' : ''}`}>
              <div className="relative w-full h-full min-h-[250px] lg:min-h-[400px]">
                <Image 
                  src={game.image} 
                  alt={game.title} 
                  fill 
                  className={`object-contain ${isStalker ? 'grayscale opacity-60' : ''}`} 
                />
              </div>
          ...
          <div className="w-full lg:w-1/2 flex flex-col relative">
            ...
            {/* Content Container */}
            <div className="relative z-10 flex-1 flex flex-col justify-center p-8 sm:p-10">
              <div className="space-y-6">
                <div className="flex items-center gap-2 text-[12px] font-mono text-red-500/80">
                  <FaTerminal /> SIM_DRILL::{game.id.toUpperCase()}
                </div>

                <h2 className={`text-4xl sm:text-5xl font-bold uppercase tracking-tighter leading-none ${isStalker ? 'text-red-600 italic' : 'text-yellow-500'}`}>
                  {game.title}
                </h2>
                  ...
                  {isStalker ? (
                    <div className="space-y-4">
                      {stalkerStatus === 'locked' && (
                        <div className="border border-red-900/50 bg-black/60 backdrop-blur-md p-8 text-center">
                          <FaSkull className="mx-auto text-red-600 mb-4 animate-bounce" size={32} />
                          <p className="text-[12px] text-red-500 uppercase tracking-[0.2em] mb-6">Cognitive Handshake Required</p>
                          <button 
                            onClick={handleBypass}
                            className="w-full py-4 bg-red-600 text-white font-bold uppercase text-sm hover:bg-red-500 transition-all shadow-[0_0_20px_rgba(220,38,38,0.4)]"
                          >
                            Bypass Protocols
                          </button>
                        </div>
                      ...
                      {stalkerStatus === 'bypassing' && (
                        <div className="py-10 text-center bg-black/80 border border-red-900/50">
                          ...
                          <p className="text-yellow-500 text-[12px] font-mono animate-pulse tracking-widest">{glitchText}</p>
                        </div>
                      )}
                      ...
                      {stalkerStatus === 'ready' && (
                        <motion.a
                          ...
                          className="flex items-center justify-center gap-3 w-full py-5 bg-red-600 text-white font-black uppercase text-base hover:bg-red-500 transition-all shadow-[0_0_30px_rgba(255,0,0,0.5)]"
                        >
                    ...
                    <a 
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full py-5 flex items-center justify-center gap-3 bg-yellow-600 text-black rounded-xl font-bold text-base transition-all hover:bg-yellow-500 shadow-xl shadow-yellow-900/20"
                    >
